#........FastSLAM algorithum implementation for crib 1.0
...
class Particle:
    #this class will create a partical for position
    #and modify it
    def __init__(self,pose):
        self.pose = pose
        #..folowing two partical will keep track all land_mark_(mu,segma)
        self.landmark_positions = []
        self.landmark_covariances = []
        #..following variable use to depricate old or errorness landmark from online map
        self.landmark_counters = []

    def number_of_landmark(self):
        # return number of landmark in current map
    ...
    def g(state,control,w):
        # this method genarate state transition matrix
        x,y,theta = state
        r,l = control
        if r != l:
            alpha = (r - l) / w
    ...
    def h_expected_measurement_for_landmark(self,landmarknumber,scanner_displacement):

        """Returns the expected distance and bearing measurement for a given
           landmark number and the pose of this particle."""
        codinate = self.h(self.pose,self.landmark_positions[landmarknumber],scanner_displacement)
        return codinate

    def H_Ql_jacobian_and_measurement_covariance_for_landmark(self,landmarknumber,Qt_measurement_covariance,scanner_displacement):
        ...
        H = self.dh_dlandmark(self.pose,self.landmark_positions[landmarknumber],scanner_displacement)
        Q_l =  np.dot(np.dot(H,self.landmark_covariances[landmarknumber]),H.T) + Qt_measurement_covariance
        return (H,Q_l)

    def lw_likelihood_of_correspondace(self,measurement,landmarknumber,Qt_neasurement_covariance,scanner_displacement):

        """For a given measurement and landmark_number, returns the likelihood
           that the measurement corresponds to the landmark."""

        delta_z = measurement - self.h_expected_measurement_for_landmark(landmarknumber,scanner_displacement)
        H,Q_l = self.H_Ql_jacobian_and_measurement_covariance_for_landmark(landmarknumber,Qt_neasurement_covariance,scanner_displacement)
        delta_Q = np.dot(np.dot(delta_z.T,np.linalg.inv(Q_l)),delta_z)
        likelihood = (1/(2*pi*sqrt(np.linalg.det(Q_l))))*np.exp( (-1/2)*delta_Q)

        return likelihood

    def compute_corespondace_likelihoods(self,measurement,number_of_landmark,Qt_measurement_covariance, scanner_displacement):


        """For a given measurement, returns a list of all correspondence
        likelihoods (from index 0 to number_of_landmarks-1)."""
        likelihoods = []
        for i in xrange(number_of_landmark):
            likelihoods.append(self.lw_likelihood_of_correspondace(measurement,i,Qt_measurement_covariance,scanner_displacement))
        return likelihoods

    def initialize_new_landmark(self,measurement_in_scanner_system,Qt_measurement_covariance,scanner_displacement):
    ...
    def scanner_to_world(self,scanner_pose,measurement_in_scanner_system):
        ...
        dx = cos(scanner_pose[2])
        dy = sin(scanner_pose[2])
        x,y = measurement_in_scanner_system
        return (x*dx - y*dy + scanner_pose[0],y*dx + x*dy + scanner_pose[1])
    ...
    def update_landmark(self, landmark_number, measurement,
        ...
        H, Q_l = self.H_Ql_jacobian_and_measurement_covariance_for_landmark(landmark_number, Qt_measurement_covariance,
                                                                        scanner_displacement)
        K = np.dot(self.landmark_covariances[landmark_number], np.dot(H.T, np.linalg.inv(Q_l)))
        self.landmark_positions[landmark_number] = self.landmark_positions[landmark_number] + np.dot(K, (
        measurement - self.h_expected_measurement_for_landmark(landmark_number, scanner_displacement)))
    ...
    def update_particle(self, measurement, measurement_in_scanner_system,
        ...
        likelihoods = self.compute_corespondace_likelihoods(measurement, number_of_landmarks, Qt_measurement_covariance,
                                                          scanner_displacement)
    ...
    def min_max_bearing(self):
        """retun maximum bearing of the scanner"""
        return (self.beam_index_angle(0),self.beam_index_angle(660))


    @staticmethod
    def beam_index_angle(i, mounting_angle=-0.06981317007977318):
        """given angle return radian value"""
...
class FastSLAM:
    ...
    def update_and_compute_weights(self, cylinders):
        ...
        for p in self.particles:
            ...
            number_of_landmarks = p.number_of_landmark()
            weight = 1.0
...
def get_motor_ticks(ticks_data,last_ticks):
    ticks = (ticks_data[3], ticks_data[7])
    motor_ticks = []
    for i in range(2):
        motor_ticks.append(ticks[i] - last_ticks[i])
    last_ticks = ticks
    print motor_ticks
    return [motor_ticks,last_ticks]
...
def main(argv):
    ...
    try:
        ...
        print "mapping_server.py -i <inputfile> -o <outputfile>"
        sys.exit(2)

    for opt,args in opts:
        if opt in ('-h','--help '):
            print 'dvdmapping_server.py -i <inputfile> -o <outputfile>'
            print '-i : File'
            print  'Robot_constant:SCANNER_DISPLACEMENT,TICKS_IN_MMS,ROBOT_WIDTH'
            sys.exit(2)
        elif opt in ( '-i','--ifile'):
            inputfile = args
            sys.exit(2)
...
if __name__  == "__main__" :
    ...
    #.......................................................
    l =  " " #incoming data # implementation of incoming data bundel connected here

    print("*******mapping server v1.0  up ***********")
    new_data_count = 0
    last_values = [0,0]
    #...................................................
    ...
    try:
        while(s.recv(4000)):
            ...
            #.....................................................
            ticks,last_values = get_motor_ticks(encode_data,last_values)
            control = map(lambda x:x*ticks_in_mms,ticks)
            fs.predict(control)
            #.....................................................
        ...
        f.close()
            ...
            #print l
            ##check wether upcoming data is new implement a method for that
            ## if coming data is Motor ticks to the predict
            ## if coming data is sensor data do th correct
        ...
        sys.exit(2)
                #.........................................
                # Data Dumping
                # data dumping and Repersentation
                #.........................................
s.close()
###########################################################

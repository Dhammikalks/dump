...
def add_obstacle(pos):
    gui.set_obstacle(world_obstacles, pos, True)
    gui.draw_background(world_obstacles, visited_nodes, optimal_path,
                           show_visited_nodes)
def remove_obstacle(pos):
    gui.set_obstacle(world_obstacles, pos, False)
    gui.draw_background(world_obstacles, visited_nodes, optimal_path,
                           show_visited_nodes)
def clear_obstacles():
    ...
    while front:
        ...
        for dx, dy, deltacost in movements:
            ...
            if (new_x >= 0 and new_x < extents[0]) and (new_y >= 0 and new_y < extents[1]):
                ...
                new_pos = (new_x, new_y)
                if visited[new_pos] == 0 and world_obstacles[new_pos] < 255:

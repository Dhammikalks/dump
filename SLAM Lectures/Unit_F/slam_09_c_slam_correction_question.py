...
class ExtendedKalmanFilterSLAM:
    ...
    def predict(self, control):
        ...
        #genarting G
        G = eye(3+2*self.number_of_landmarks,3+2*self.number_of_landmarks)
        G[0:3,0:3] = G3
        #genarate the covariance
        self.covariance = dot(G,dot(self.covariance,G.T))+ R
        #genarating new state
    ...
    def add_landmark_to_state(self, initial_coords):
        ...
        # genarating new covarince metrix
        G = 1e2 * eye(3 + 2 * self.number_of_landmarks)
        print(self.covariance)
        G[0:3 + 2 * (self.number_of_landmarks - 1), 0:3 + 2 * (self.number_of_landmarks - 1)] = self.covariance

        self.covariance = G

        # genarating new stat vector
        S = zeros((3+2*self.number_of_landmarks))
        S[0:3+2*(self.number_of_landmarks - 1)] = self.state
        S[3+2*(self.number_of_landmarks - 1):len(S)] = initial_coords

        self.state = S;
        return (self.number_of_landmarks-1)  # Replace this.
    ...
    def correct(self, measurement, landmark_index):
        ...
        H[0:2,0:3] = H3
        for h in xrange(self.number_of_landmarks):
            H[0:2,3+2*(h-1):5+2*(h-1)] = -H[0:2,0:2]
        print(H)

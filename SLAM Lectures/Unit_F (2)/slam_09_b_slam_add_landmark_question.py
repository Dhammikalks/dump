...
class ExtendedKalmanFilterSLAM:
    ...
    def predict(self, control):
        ...
        R = zeros((2*self.number_of_landmarks+3 ,2*self.number_of_landmarks+3 ))
        G  = eye(2*self.number_of_landmarks+3)
        #print("number of lanmarks")
        #print(self.number_of_landmarks)
        R[0:3,0:3] = R3
        G[0:3,0:3] = G3
    ...
    def add_landmark_to_state(self, initial_coords):
        ...
        #        covariance matrix.
        st = zeros((3+2*self.number_of_landmarks))
        st[0:3+2*(self.number_of_landmarks-1)] = self.state
        st[3+2*(self.number_of_landmarks -1):len(st)] = initial_coords
        self.state = st
        #print(self.state)
        #self.state = (self.state,initial_coords)

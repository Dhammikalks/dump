...
def filter_step(old_pose, motor_ticks, ticks_to_mm, robot_width,
    ...
    if motor_ticks[0] == motor_ticks[1]:
        ...
        x_old = old_pose[0] - scanner_displacement*cos(old_pose[2])
        y_old = old_pose[1] - scanner_displacement*sin(old_pose[2])

        alpha = (motor_ticks[1] - motor_ticks[0]) * ticks_to_mm / robot_width
        R = motor_ticks[0] * ticks_to_mm / alpha
        theta = (old_pose[2] + alpha) % (2 * pi)

        x = x_old - (R + robot_width / 2) * sin(old_pose[2]) + (R + robot_width / 2) * sin(theta)
        y = y_old + (R + robot_width / 2) * cos(old_pose[2]) - (R + robot_width / 2) * cos(theta)

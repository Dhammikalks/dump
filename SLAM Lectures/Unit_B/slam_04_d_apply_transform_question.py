...
def find_cylinder_pairs(cylinders, reference_cylinders, max_radius):
    ...
    for i in xrange(len(cylinders)):
        ...
        for j in xrange(len(reference_cylinders)):
            len_j = ((cylinders[i][0] - reference_cylinders[j][0]) ** 2 + (
            cylinders[i][0] - reference_cylinders[j][0]) ** 2) ** 0.5
            if len_j < min_len:
                min_len = len_j
                I = i
                J = j
...
def estimate_transform(left_list, right_list, fix_scale = False):
    ...
    # --->>> Insert your previous solution here.
    cs, ss, rr, ll = 0, 0, 0, 0
    norm_left_x, norm_left_y, norm_right_x, norm_right_y = 0, 0, 0, 0

    for i in xrange(len(left_list)):
        norm_left_x = left_list[i][0] - lc[0]
        norm_left_y = left_list[i][1] - lc[1]

        norm_right_x = right_list[i][0] - rc[0]
        norm_right_y = right_list[i][1] - rc[1]

        cs += norm_right_x * norm_left_x + norm_right_y * norm_left_y
        ss += -norm_right_x * norm_left_y + norm_right_y * norm_left_x
        rr += norm_right_x * norm_right_x + norm_right_y * norm_right_y
        ll += norm_left_x * norm_left_x + norm_left_y * norm_left_y
    ...
    if (rr == 0 or ll == 0):
        ...
        if (fix_scale):
            ...
            la = (rr / ll) ** (0.5)
        lenght = (cs ** 2 + ss ** 2) ** (0.5)
        t = (rr / ll) ** (0.5)
        c = cs / lenght
        s = ss / lenght
        tx = rc[0] - t * c * lc[0] + t * s * lc[1]
        ty = rc[1] - t * s * lc[0] - t * c * lc[1]
        #print(la, c, s, tx, ty)
        return la, c, s, tx, ty
...
def correct_pose(pose, trafo):
    ...
    x_new = la*c*pose[0]-la*s*pose[1]+tx
    y_new = la*s*pose[0]+la*c*pose[1]+ty
    theta_new = (pose[2]+ atan2(s,c))%(2*pi)
    print(x_new, y_new, theta_new)
    return (x_new,y_new,theta_new)  # Replace this by the corrected pose.

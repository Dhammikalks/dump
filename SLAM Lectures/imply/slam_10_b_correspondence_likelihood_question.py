...
class Particle:
    ...
    def H_Ql_jacobian_and_measurement_covariance_for_landmark(
        ...
        Ql = np.dot(np.dot(H,self.landmark_covariances[landmark_number]),H.T) + Qt_measurement_covariance # Replace this.
        return (H, Ql)
    ...
    def wl_likelihood_of_correspondence(self, measurement,
        ...
        delt_Q = np.dot(np.dot(delta_z.T,np.linalg.inv(Q_l)),delta_z)
        likelyhood = (1/(2*pi*sqrt(np.linalg.det(Q_l)))) *np.exp((-1/2)*delt_Q)
        #print(likelyhood)

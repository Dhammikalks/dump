...
class Particle:
    ...
    def h_expected_measurement_for_landmark(self, landmark_number,
        ...
        scanner_pose = (self.pose[0] + cos(self.pose[2]) * scanner_displacement,
                        ...
                        self.pose[2])
        codinate = LegoLogfile.scanner_to_world(scanner_pose, self.landmark_positions[landmark_number])
    ...
    def update_landmark(self, landmark_number, measurement,
        ...
        self.landmark_covariances[landmark_number] = np.dot((np.eye(2,2)-np.dot(K,H)),self.landmark_covariances[landmark_number])
...
if __name__ == '__main__':
    ...
    # Measure second landmark.
    # Assume we would measure a slightly different range.
    measurement = np.array([1000.0,1000.0])/sqrt(2)
    p.update_landmark(2, measurement, Qt_measurement_covariance,
                      scanner_displacement)
